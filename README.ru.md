...
*   **Горячие клавиши ПК и мягкая перезагрузка:** Нативные триггеры для `Ctrl+Alt+Del`, `Win+L` (Блокировка экрана), `Ctrl+Shift+Esc` (Диспетчер задач) и `Alt+F4`. Реализован безопасный макрос штатной перезагрузки ОС (`Win+R` -> `shutdown /r /t 0` -> `Enter`) с отдельной защищенной кнопкой «Restart PC» и подтверждением действия.
...
*   **Обновленный Web-интерфейс с динамическим опросом:** Удобная компоновка элементов на вкладке ПК (питание сверху, хоткеи в центре, строка ввода снизу) и фоновый опрос (`/api/status`), обновляющий индикатор хоста на лету без перезагрузки веб-страницы.
...
*   **Управление ПК и макросы:** `wake`, `sleep`, `power`, `enter`, `reboot_pc`.
*   **Горячие клавиши ПК:** `ctrl_alt_del`, `win_l`, `taskmgr`, `alt_f4`.
...
2.  **Диспетчер устройств Windows:**
    *   В разделе **Клавиатуры** $\rightarrow$ `Logitech Total Keyboard V1.4` $\rightarrow$ **Свойства** $\rightarrow$ вкладка **Управление электропитанием**.
